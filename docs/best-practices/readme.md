---
title: Best Practices
doc_type: readme
status: active
last_updated: 2026-09-10
---
# Best Practices

Each specialty has one canonical `readme.md` containing its guidance. Add new
guidance to that file rather than creating a second README under another name.

## Specialties

- [Software Architecture](software-architecture/readme.md)
- [Security](security/readme.md)
- [Cloud Architecture](cloud-architecture/readme.md)
- [DevOps & Platform Engineering](devops/readme.md)
- [Data & Analytics](data-analytics/readme.md)
- [Integration & APIs](integration/readme.md)
- [Observability](observability/readme.md)

## Supporting Documents

- [Specialty Template](templates/readme.md)
- [Solution Architect Radar (2025 Q4)](radar.md)
- [Architecture Governance Reviews](../reviews/readme.md)
- [Architecture Decision Records](../adr/index.md)
- [Architecture Playbook](../index.md)
