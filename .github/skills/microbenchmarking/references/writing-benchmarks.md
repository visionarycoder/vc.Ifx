---
title: Benchmark authoring techniques
doc_type: reference
status: active
...
related_skills:
  - microbenchmarking
appliesTo: '**/*.{cs,csproj,json,md,txt,log}'
tags:
  - microbenchmarking
  - reference
---
...
[GlobalSetup]
public void Setup() => array = Enumerable.Range(0, 1000).ToArray();
...
**Note**: when `[MemoryRandomization]` is enabled, BDN re-runs `[GlobalCleanup]` and `[GlobalSetup]` after every iteration (overriding the normal once-only behavior). See [bdn-internals-and-tuning.md](bdn-internals-and-tuning.md) for details.
...
private int[] data, scratch;

[GlobalSetup]
public void Setup() => data = Enumerable.Range(0, Size).Reverse().ToArray();

[IterationSetup]
public void ResetArray() => Array.Copy(data, scratch = new int[Size], Size);

[Benchmark]
public void Sort() => Array.Sort(scratch);
```
...
```csharp
private byte[] array = new byte[18];

[Benchmark(OperationsPerInvoke = 16)]
public Span<byte> Slice()
{
    Span<byte> span = new Span<byte>(array); // setup: can't go in GlobalSetup (stack-only)
...
[Benchmark]
public void Add() => numbers.Add(12345);
```
...
[Benchmark]
public IEnumerable<int> WhereQuery() => numbers.Where(n => n > 50);

// Correct — materialize the result so the query actually executes
[Benchmark]
public List<int> WhereQuery() => numbers.Where(n => n > 50).ToList();
```
...
[Benchmark]
public void WhereQuery() => numbers.Where(n => n > 50).Consume(consumer);
```
...
public class CollectionBenchmark<T>
{
    ...
    [GlobalSetup]
    public void Setup() => list = new List<T>(Enumerable.Repeat(default(T)!, 1000));

    [Benchmark]
    public bool Contains() => list.Contains(default!);
}
```
