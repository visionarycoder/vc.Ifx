...
public class SortBenchmark
{
    ...
    [GlobalSetup]
    public void Setup() => data = Enumerable.Range(0, 1000).Reverse().ToArray();

    [Benchmark(Baseline = true)]
    public void BubbleSort() => Sorting.BubbleSort((int[])data.Clone());

    [Benchmark]
    public void QuickSort() => Sorting.QuickSort((int[])data.Clone());
}
```
...
public void Setup()
{
    ...
    data = Enumerable.Range(0, N).OrderBy(_ => rng.Next()).ToArray();
}

[Benchmark]
public void Sort() => Array.Sort((int[])data.Clone());
```
