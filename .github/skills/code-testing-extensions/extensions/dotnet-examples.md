---
title: .NET Pipeline Examples
doc_type: reference
status: active
last_updated: 2026-07-26
target_audience: ai
complexity: high
estimated_tokens: 2805
prerequisites:
  - code-testing-agent
related_skills:
  - code-testing-agent
...
{
    private readonly Mock<IInvoiceRepository> repositoryMock = new();
    private readonly InvoiceService sut;

    public InvoiceServiceTests()
    {
        sut = new InvoiceService(repositoryMock.Object);
    }
        ...
        int quantity, double unitPrice, double taxRate, double expected)
    {
        ...
        // Act
        var total = sut.CalculateTotal(invoice);
    ...
    {
        Assert.ThrowsExactly<ArgumentNullException>(() => sut.CalculateTotal(null!));
    }
    ...
    public void CalculateTotal_EmptyLineItems_ThrowsInvalidOperationException()
    {
        ...
        // Act & Assert
        Assert.ThrowsExactly<InvalidOperationException>(() => sut.CalculateTotal(invoice));
    }
    ...
    public async Task GetByIdAsync_ExistingId_ReturnsInvoice()
    {
        ...
        repositoryMock.Setup(r => r.FindAsync(42)).ReturnsAsync(expected);
    ...
    {
        // Arrange
        repositoryMock.Setup(r => r.FindAsync(999)).ReturnsAsync((Invoice?)null);

        // Act & Assert
        await Assert.ThrowsExactlyAsync<KeyNotFoundException>(
            () => sut.GetByIdAsync(999));
    }
    ...
    public async Task MarkAsPaidAsync_UnpaidInvoice_SetsStatusAndDate()
    {
        ...
        repositoryMock.Setup(r => r.FindAsync(1)).ReturnsAsync(invoice);

        // Act
        await sut.MarkAsPaidAsync(1);

        // Assert
        Assert.AreEqual(InvoiceStatus.Paid, invoice.Status);
        Assert.IsNotNull(invoice.PaidDate);
        repositoryMock.Verify(r => r.UpdateAsync(invoice), Times.Once);
    }
