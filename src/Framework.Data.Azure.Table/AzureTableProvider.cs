...
/// </summary>
public sealed class AzureTableProvider : IAzureTableProvider
{
    private readonly AzureTableOptions options;
    private readonly TableServiceClient tableServiceClient;
    private readonly TableClient tableClient;
    private readonly ILogger<AzureTableProvider> logger;

    public AzureTableProvider(AzureTableOptions options, ILogger<AzureTableProvider> logger)
    {
    ...
    public async Task<bool> TableExistsAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            bool exists = false;
            await foreach (TableItem? item in tableServiceClient.QueryAsync(filter: $"TableName eq '{options.TableName}'", cancellationToken: cancellationToken).ConfigureAwait(false))
            {
                exists = true;
                break;
            }
    ...
    public async Task UpdateEntityAsync<T>(T entity, ETag etag = default, TableUpdateMode mode = TableUpdateMode.Replace, CancellationToken cancellationToken = default) where T : class, ITableEntity
    {
        ...
        try
        {
            ...
            await tableClient.UpdateEntityAsync(entity, etag == default ? ETag.All : etag, mode, cancellationToken).ConfigureAwait(false);
    ...
    public async Task DeleteEntityAsync(string partitionKey, string rowKey, ETag etag = default, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await tableClient.DeleteEntityAsync(partitionKey, rowKey, etag == default ? ETag.All : etag, cancellationToken).ConfigureAwait(false);
        ...
        where T : class, ITableEntity
    {
        try
        {
            ...
            await foreach (T? entity in query.ConfigureAwait(false))
            {
                results.Add(entity);
            }
    ...
    public async Task SubmitBatchAsync(IEnumerable<TableTransactionAction> actions, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await tableClient.SubmitTransactionAsync(actionList, cancellationToken).ConfigureAwait(false);
        ...
        where T : class, ITableEntity
    {
        ...
        string filter = $"PartitionKey eq '{partitionKey}'";
        return await QueryEntitiesAsync<T>(filter, cancellationToken: cancellationToken).ConfigureAwait(false);
    }
