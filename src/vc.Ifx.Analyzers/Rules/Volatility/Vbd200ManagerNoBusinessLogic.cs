...
public static class Vbd200ManagerNoBusinessLogic
{
    public static readonly DiagnosticDescriptor Rule = new(
        ...
        title: "Managers must not contain business logic", "Manager '{0}' contains business logic", "Architecture", DiagnosticSeverity.Warning, isEnabledByDefault: true, helpLinkUri: "https://github.com/visionarycoder/vc.Ifx/blob/main/docs/roslyn/diagnostic-catalog.md#legacy-vbd-policy");
    ...
    private static void AnalyzeMethod(SyntaxNodeAnalysisContext context)
    {
        ...
        var containsBusinessLogic = nodes.Any(n => NodeIsBusinessLogic(n, context.SemanticModel, context.CancellationToken));

        if (!containsBusinessLogic)
            return;

        var managerName = methodSymbol.ContainingType.Name;
    ...
    private static bool NodeIsBusinessLogic(SyntaxNode node, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        switch (node)
        {
            ...
            case BinaryExpressionSyntax arithmetic when ArithmeticKinds.Contains(arithmetic.Kind()):
                return IsDecimalArithmetic(arithmetic, semanticModel, cancellationToken) && !OnlyTouchesCrossCuttingInfrastructure(arithmetic, semanticModel, cancellationToken);
            case AssignmentExpressionSyntax assign when ArithmeticKinds.Contains(assign.Kind()):
                return IsDecimalArithmetic(assign, semanticModel, cancellationToken) && !OnlyTouchesCrossCuttingInfrastructure(assign, semanticModel, cancellationToken);
            ...
            case BinaryExpressionSyntax comparison when ComparisonKinds.Contains(comparison.Kind()):
                return TouchesBusinessMember(comparison.Left, semanticModel, cancellationToken) && TouchesBusinessMember(comparison.Right, semanticModel, cancellationToken);

            // Aggregation over, or conditional projection of, domain data.
            case InvocationExpressionSyntax invocation:
                return IsBusinessAggregation(invocation, semanticModel, cancellationToken) || IsComputedProjection(invocation, semanticModel, cancellationToken);
    ...
    private static bool IsBusinessAggregation(InvocationExpressionSyntax invocation, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        if (GetInvokedMethodName(invocation, semanticModel, cancellationToken) is not { } name || !AggregationMethods.Contains(name))
            return false;

        if (!ProducesDecimal(invocation, semanticModel, cancellationToken))
            return false;

        var arguments = invocation.ArgumentList.Arguments;
        if (arguments.Count > 0)
            return arguments.Any(argument => TouchesBusinessMember(argument.Expression, semanticModel, cancellationToken));

        return invocation.Expression is MemberAccessExpressionSyntax memberAccess
            && TouchesBusinessMember(memberAccess.Expression, semanticModel, cancellationToken);
    }
    ...
    private static bool IsComputedProjection(InvocationExpressionSyntax invocation, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        if (GetInvokedMethodName(invocation, semanticModel, cancellationToken) is not ("Select" or "SelectMany"))
            return false;
        ...
        return invocation.ArgumentList.Arguments.Any(argument =>
            ...
            && TouchesBusinessMember(argument.Expression, semanticModel, cancellationToken));
    }

    private static string? GetInvokedMethodName(InvocationExpressionSyntax invocation, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
    ...
    private static (bool touchesInfrastructure, bool touchesBusiness) ClassifyMembers(ExpressionSyntax expression, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        ...
        foreach (var node in expression.DescendantNodesAndSelf())
        {
            ...
            if (symbol is IMethodSymbol { MethodKind: MethodKind.BuiltinOperator or MethodKind.UserDefinedOperator or MethodKind.AnonymousFunction })
                continue;
    ...
    private static bool OnlyTouchesCrossCuttingInfrastructure(ExpressionSyntax expression, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        var (touchesInfrastructure, touchesBusiness) = ClassifyMembers(expression, semanticModel, cancellationToken);
        return touchesInfrastructure && !touchesBusiness;
    }

    // True when an expression references at least one business member (a domain property/field/method).
    private static bool TouchesBusinessMember(ExpressionSyntax expression, SemanticModel semanticModel, CancellationToken cancellationToken)
        => ClassifyMembers(expression, semanticModel, cancellationToken).touchesBusiness;
    ...
    private static bool ProducesDecimal(InvocationExpressionSyntax invocation, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        ...
        foreach (var argument in invocation.ArgumentList.Arguments)
            if (argument.Expression is LambdaExpressionSyntax { Body: ExpressionSyntax selectorBody }
                && IsDecimal(semanticModel.GetTypeInfo(selectorBody, cancellationToken).Type))
                return true;
    ...
    private static bool IsDecimalArithmetic(ExpressionSyntax expression, SemanticModel semanticModel, CancellationToken cancellationToken)
    {
        if (IsDecimal(semanticModel.GetTypeInfo(expression, cancellationToken).Type))
            return true;

        return expression switch
        {
            BinaryExpressionSyntax binary => IsDecimal(semanticModel.GetTypeInfo(binary.Left, cancellationToken).Type) || IsDecimal(semanticModel.GetTypeInfo(binary.Right, cancellationToken).Type),
            _ => IsDecimal(semanticModel.GetTypeInfo(((AssignmentExpressionSyntax)expression).Left, cancellationToken).Type) || IsDecimal(semanticModel.GetTypeInfo(((AssignmentExpressionSyntax)expression).Right, cancellationToken).Type)
        };
    }

    private static bool IsCrossCuttingInfrastructure(ISymbol symbol)
    {
        if (ProjectAnalyzer.GetLayer(symbol.ContainingAssembly.Name) == Layer.Infrastructure)
            return true;
